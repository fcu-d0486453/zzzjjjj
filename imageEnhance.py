...
class ImEnhance:
    ...
    """
    def __init__(self):
        self._rotation_degree = None
        # enhance 的 seq
        self._rotation_degree = randint(-45, 45)
        self.enhance_event = [
            iaa.Affine(rotate=self._rotation_degree),  # 隨機旋轉 A~B
    ...
    def augument(self, fn, seq, args, gen_number=1):
    ...
    def get_seq(self, random_order=False, random_pick=False, pick=2, image_ref_size=None):
        """ 取得強化用的 sequence。
        Args:
        ...
        if not random_pick:
            if random_order:
                return iaa.Sequential(self.enhance_event, random_order=random_order)
            else:
                return iaa.Sequential(self.enhance_event)
        else:
            assert len(self.enhance_event) >= pick
            if pick < 0:
                ...
                return iaa.SomeOf(pick, self.enhance_event)
